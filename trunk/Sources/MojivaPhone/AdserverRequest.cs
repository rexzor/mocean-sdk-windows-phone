...
namespace MojivaPhone
{
    ...
    internal class AdserverRequest: Object
    {
	    ...
	    private String parameter_ip = "ip";
        ...
        protected void SetupAutodetectParameters()
        {
            ...
            if (!parameters.ContainsKey(parameter_ip)) parameters.Add(parameter_ip, "");
//             if (!parameters.ContainsKey(parameter_latitude)) parameters.Add(parameter_latitude, "");
//             if (!parameters.ContainsKey(parameter_longitude)) parameters.Add(parameter_longitude, "");
        ...
        /// <returns>AdserverRequest</returns>
	    public AdserverRequest setIp(String ip) {
	    ...
	    public String getIp() {
            if (parameters.ContainsKey(parameter_ip) && !String.IsNullOrEmpty(parameters[parameter_ip]))
                return parameters[parameter_ip];
            else if (!String.IsNullOrEmpty(AutoDetectParameters.Instance.IP))
            {
                return AutoDetectParameters.Instance.IP;
            }
	    ...
	    public String getUa() {
            if (parameters.ContainsKey(parameter_userAgent) && !String.IsNullOrEmpty(parameters[parameter_userAgent])) return parameters[parameter_userAgent];
            else {
/*
                RegistryKey key = Registry.LocalMachine.OpenSubKey("Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings\\5.0\\User Agent\\Templates\\Same as Pocket PC");

                if (key != null)
                {
                    String val = key.GetValue("", "").ToString() + " " + "(" + key.GetValue("Compatible", "").ToString() + "; " + key.GetValue("Version", "") + "; " + key.GetValue("Platform", "") + ")";
                    return val;
                }
*/
...
/*            if (parameters.ContainsKey(parameter_latitude) && !String.IsNullOrEmpty(parameters[parameter_latitude]))
                return parameters[parameter_latitude];
            else if (AutoDetectParameters.Instance.Latitude != 0)
            {
                return AutoDetectParameters.Instance.Latitude.ToString();
            }
            return String.Empty;
 */
...
/*            if (parameters.ContainsKey(parameter_longitude) && !String.IsNullOrEmpty(parameters[parameter_longitude]))
                return parameters[parameter_longitude];
            else if (AutoDetectParameters.Instance.Longitude != 0)
            {
                return AutoDetectParameters.Instance.Longitude.ToString();
            }
            return String.Empty;
 */
